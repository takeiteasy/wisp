...
#
# One fresh `node` process per file on purpose: the wisp compiler's gensym
# counter is process-global and is not reset between compile() calls, so
# compiling several modules in one process would drift.
set -eu
...
ENTRY="$BOOTSTRAP_DIR/wisp-bootstrap.js"
...
[ -f "$ENTRY" ] || { echo "missing $ENTRY (run: make $ENTRY)" >&2; exit 1; }
